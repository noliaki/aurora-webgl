...
void main(void){
  ...
  vec3 rgb_color = vec3(
    (snoise(vec3(p.x / 3.0, p.y / 3.0, u_time / 1.1)) + 1.0) / 2.0 + u_rgbOffset.x + offset,
    (snoise(vec3(p.x / 2.0, p.y / 2.0, u_time / 1.2)) + 1.0) / 2.0 + u_rgbOffset.y + offset,
    (snoise(vec3(p.x / 5.0, p.y / 5.0, u_time / 1.3)) + 1.0) / 2.0 + u_rgbOffset.z + offset
  );
